...
void ChatCmd_Help(const char* line, const int argc, char **const argv)
{
	...
	ZChatOutput("/myfps - Fps output", ZChat::CMT_SYSTEM);
	ZChatOutput("/report <player> <reason> - Report a player and a reason", ZChat::CMT_SYSTEM);
	ZChatOutput("F9 - HP/AP output", ZChat::CMT_SYSTEM);
	...
	ZChatOutput("[NS] - No Melee", ZChat::CMT_SYSTEM);
	...
	ZChatOutput("[ND] - No donor/event items", ZChat::CMT_SYSTEM);
}
...
void ChatCmd_AdminAnnounce(const char* line, const int argc, char **const argv)
{
	...
	char szMsg[256];
	sprintf(szMsg,"[Anouncement]%s : %s",ZGetMyInfo()->GetCharName(), argv[1]);
